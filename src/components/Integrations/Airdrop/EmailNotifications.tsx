...
const EmailNotifications = () => {
  ...
  const {
    ...
    mutationFn: async (args: { nextEmail: string; fullName?: string }) => {
      ...
      const settings: SettingsUpdate = buildSubscribeSettings(
        args.nextEmail,
        args.fullName,
      );
  ...
  return (
    <div className="mx-auto w-full max-w-[1095px] rounded-[30px] bg-gradient-to-br from-[#BE75FF] to-[#F9BFCE] p-[1px]">
      <div className="bg-primaryBackground rounded-[29px] p-2 lg:p-5">
        {/* Header */}
        ...
        {/* Title */}
        <h2 className="text-primaryText mb-4 text-2xl font-semibold">
          Subscribe to Get Notified About Earning Opportunities
        </h2>

        {/* Description */}
        <p className="text-primaryText mb-4 text-sm leading-relaxed">
          Get real-time alerts when new juror opportunities go live.
        </p>

        <p className="text-secondaryText mb-6 text-sm leading-relaxed">
          Why Subscribe? Because missing a juror draw means missing rewards,
          reputation, and your chance to help shape decentralized justice.
        </p>
        ...
        <div className="space-y-4 px-4 py-2 lg:px-0 lg:py-0">
          <div>
            <label
              htmlFor="email"
              className="text-orange mb-2 block text-sm font-medium uppercase"
            >
              EMAIL
            </label>
            <div className="flex flex-col sm:flex-row">
              <Field
                ...
                className={`text-primaryText placeholder-secondaryText flex-1 rounded-lg border-2 px-4 py-3 focus:outline-none ${isEmailValid ? "border-orange focus:border-purple" : "border-red-500 focus:border-red-500"}`}
              />
              <ActionButton
                onClick={handleSubscribe}
                label="Subscribe"
                disabled={
                  !address || !trimmedEmail || !isEmailValid || isFetching
                }
                isLoading={isSubmitting}
                className="mb-2 px-8 lg:mb-0"
                variant="primary"
              />
            </div>
            {!isEmailValid && (
            ...
            {isSuccess && (
              <div className="mt-2 flex items-center gap-2">
                <CheckCircle className="h-4 w-4 fill-green-500" />
