...
/**
 * The reward is locked in and will be paid automatically: referee verified,
 * review clean, nobody flagged, payout not yet confirmed.
 */
export const isRewardPending = (user: ReferredUser): boolean =>
  user.payoutStatus !== PohReferralPayoutTransactionStatus.Confirmed &&
  !user.refereeFlagged &&
  (user.reviewStatus === PohReferralReviewStatus.Active ||
    user.reviewStatus === PohReferralReviewStatus.Approved) &&
