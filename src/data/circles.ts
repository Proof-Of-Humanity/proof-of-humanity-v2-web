...
export const getProcessedCirclesData = async (
  address: string,
): Promise<ProcessedCirclesData> => {
  if (!address) {
  ...
  try {
    ...
    const humanityId =
      data.registrations?.[0]?.id || data.crossChainRegistrations?.[0]?.id;
    ...
    if (!primaryCircleAccount) {
      const humanityData = await sdk[
        circlesChainId
      ].GetHumanityWithCircleAccountById({
        humanityId,
      });
    ...
    const linkStatus =
      !isNaN(expiryMs) && expiryMs > Date.now() ? "linked" : "expired";
...
export async function validateCirclesHumanity(
  ...
  if (!contractInfo.address) {
    throw new Error(
      `CirclesHub contract not deployed on chain ${circlesChainId}`,
    );
  }
