...
const humanityQuery = gql`
  query GetHumanities($first: Int, $skip: Int, $now: String!) {
    humanities(
      first: $first
      skip: $skip
      where: { registration_: { expirationTime_gt: $now } }
    ) {
      id
      registration {
        expirationTime
...
async function updateStateEthGnosis() {
  if (!rpcUrl || !privateKey) {
    throw new Error(
      "Missing MAINNET_RPC or PRIVATE_KEY in environment variables",
    );
  }
  ...
  const contractData = await mainnetClient.request<{
    crossChainGateways: { id: string }[];
  }>(contractQuery);
  ...
  const ccpohAddress =
    info.address ||
    ("0xa478095886659168E8812154fB0DE39F103E74b2" as `0x${string}`);
  ...
  while (true) {
    const res = await mainnetClient.request<{
      humanities: { id: string; registration: { expirationTime: string } }[];
    }>(humanityQuery, { first: batchSize, skip, now: now.toString() });
  ...
  while (true) {
    const res = await gnosisClient.request<{
      crossChainRegistrations: {
        id: string;
        expirationTime: string;
        claimer: { id: string };
      }[];
    }>(gnosisQuery, { first: batchSize, skip });
    if (
      !res.crossChainRegistrations ||
      res.crossChainRegistrations.length === 0
    )
      break;
  ...
  for (let i = 0; i < profilesToUpdate.length; i += TX_BATCH_SIZE) {
    ...
    console.log(
      `[Batch ${batchIndex}/${totalBatches}] Processing ${batchProfiles.length} profiles...`,
    );
  ...
  console.log(
    `Total Estimated Gas Cost for all updates: ${formatEther(totalGasCost)} ETH`,
  );
  console.log(
    `[${new Date().toISOString()}] Completed update-state-eth-gnosis scheduled function`,
  );
}
...
export default async (req: Request, context: Context) => {
  try {
    console.log(
      "Function update-state-eth-gnosis triggered at:",
